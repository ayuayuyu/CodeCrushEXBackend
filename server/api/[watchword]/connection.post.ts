...
export default defineEventHandler(async (event) => {
    try {
      ...
      const body = await readBody<{player: number}>(event);

      // バリデーション: player が指定されていない場合
    if (!body?.player || (body.player !== 1 && body.player !== 2)) {
    ...
    // プレイヤー情報を更新または挿入
    const existing = db
      .prepare("SELECT * FROM watchwords WHERE watchword = ?")
      .get(watchword);
